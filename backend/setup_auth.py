"""初回セットアップ: Gmail の OAuth2 認証を実行する。

Sheets の認証（token.json / SHEETS_TOKEN_JSON）は ISS-519 で撤去した。
Sheets の読み書きは GAS（gas/Code.gs）が行う。
...
権限を許可すると gmail_token.json が生成される。
"""
import io
import pickle
import sys
...
def main():
    ...
    if ok:
        ...
        print()
        print("次のステップ:")
        print("1. config.yaml のメールアドレスを設定")
